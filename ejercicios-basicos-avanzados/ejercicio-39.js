...
Para ello recorre la lista de usuarios y usa forin para recoger el nombre de los sonidos que el 
usuario tenga como favoritos. 
...
const countFavouriteSounds = (userList) => {
    let favouriteCounter = {}
    for (user of userList){
        for(sound in user.favoritesSounds)
            if(!favouriteCounter[sound]){
                favouriteCounter[sound]=1
            }else favouriteCounter[sound]+=1
    }return favouriteCounter
}
console.log(countFavouriteSounds(users))