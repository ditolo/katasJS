/* Filtra una lista de canciones para crear una playlist que solo contenga canciones de rock que 
...
//Vamos a utilizar un filtro por un campo que pasamos por parámetro y otro valor adicional de duración mínima
function filterTwoFields(list, field1, value1, minDuration)
{
    let filtered1=[];
    let filtered1and2=[];
    for (element of list){
        if (element[field1]===value1)filtered1.push(element)
    }
    for(element of filtered1){
        if(element['duration']>minDuration)filtered1and2.push(element)
    }
    for(song of filtered1and2)
    {
        rockPlaylist.push(song)
    }
    return rockPlaylist;
}
console.log(filterTwoFields(songs,"genre","Rock",5))